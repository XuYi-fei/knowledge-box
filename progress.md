...
- 用户侧聊天引用打开文档详情已改为同窗口路由跳转，不再新开浏览器窗口；文档详情页顶部已有“返回对话”入口，因此单窗口内即可完成“查看详情 -> 返回会话”的往返。
...
- 用户侧文档详情页大纲已改为以“实际渲染出的正文标题 DOM”为唯一数据源：页面渲染后从 `.admin-markdown h1-h6` 反向生成大纲并写回稳定 `id`，右侧树、点击跳转、hash 与滚动高亮全部基于同一组真实节点，不再依赖原始 Markdown 预解析顺序。
- 用户侧文档详情页已增加 `debugOutline=1` 调试面板：可记录真实标题大纲生成、初始定位解析、点击跳转命中、目标元素位置与 `activeHeading` 变化，便于直接采集页面内日志定位大纲/锚点问题。
- 用户侧文档详情页已改为在渲染阶段显式注入稳定 `headingIds`，同时 DOM 反向大纲仅复用现有标题 `id`；避免因后续 React 重渲染覆盖运行期写入的 `id`，导致点击日志里 `targetFound=false`、滚动高亮失效。
...
- 前端验证：`npm --prefix frontend run build` 通过（含聊天引用改为同窗口跳转到文档详情页，不再 `window.open` 新窗口）。
...
- 前端验证：`npm --prefix frontend run build` 通过（含文档详情页从真实 DOM 标题反向生成大纲，以及 `history.pushState` 替代原生 `location.hash` 跳转）。
- 前端验证：`npm --prefix frontend run build` 通过（含文档详情页 `debugOutline=1` 页面内调试日志面板）。
- 前端验证：`npm --prefix frontend run build` 通过（含文档详情页稳定 `headingIds` 注入，修复重渲染后标题锚点丢失）。
