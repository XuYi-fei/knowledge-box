...
usage() {
  cat <<'USAGE'
Usage: deploy/bin/start-backend-flat.sh [options]
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    *)
      echo "[start-backend-flat] Unknown option: $1" >&2
      usage >&2
      exit 1
      ;;
  esac
...
mkdir -p "${LOG_DIR}" "${TMP_DIR}" "${UPLOADS_DIR}"

if [[ ! -f "${BASE_DIR}/knowledge-box-backend.jar" ]]; then
  echo "[start-backend-flat] Missing ${BASE_DIR}/knowledge-box-backend.jar" >&2
  exit 1
fi
...
export KB_STORAGE_LOCAL_BASE_PATH=${KB_STORAGE_LOCAL_BASE_PATH:-${UPLOADS_DIR}}
...
JAVA_CMD=(
  java
  "${JAVA_OPTS_ARRAY[@]}"
  -jar "${BASE_DIR}/knowledge-box-backend.jar"
...
if [[ "${RUN_MODE}" == "daemon" ]]; then
  if [[ -f "${PID_FILE}" ]]; then
    ...
    if [[ -n "${EXISTING_PID}" ]] && kill -0 "${EXISTING_PID}" 2>/dev/null; then
      echo "[start-backend-flat] Backend already running. pid=${EXISTING_PID} pidFile=${PID_FILE}" >&2
      exit 1
    fi
    rm -f "${PID_FILE}"
  fi
  ...
  PID=$!
  echo "${PID}" > "${PID_FILE}"
  echo "[start-backend-flat] Started in background. pid=${PID} pidFile=${PID_FILE} log=${LOG_DIR}/backend.out.log"
  exit 0
fi

echo "[start-backend-flat] Starting in foreground. log=${LOG_DIR}/backend.out.log profile=${SPRING_PROFILES_ACTIVE} bootstrapDir=${KB_DOCUMENT_BOOTSTRAP_SEED_DIRECTORY}" >&2
