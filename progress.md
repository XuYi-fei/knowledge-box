...
- 已压缩公开文库文章详情顶部元信息区：分类/来源文件/专栏/最近更新优先单行紧凑展示，标签独占一行，减少正文阅读区被元信息占用的高度。
...
- 已修复管理端公共布局滚动链：`AdminLayout` 现改为内容区独立滚动，`知识文档` 与 `文档审核` 页面在关闭窗口级滚动后仍可正常向下滚动。
...
- 前端：`npm --prefix frontend run build` 可通过，已覆盖公开文库文章详情顶部元信息区压缩布局修复。
...
- 前端：`npm --prefix frontend run build` 可通过，已覆盖本次 `AdminLayout` 内容滚动修复。
