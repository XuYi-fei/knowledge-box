...
- `backend/src/main/java/com/knowledgebox/web`、`service`、`repository`、`domain`、`config`、`security` 是当前真实生效的后端运行时代码。
- `backend/src/main/java/com/knowledgebox/app/*` 目录目前存在，但没有活跃代码文件。维护时不要把它误判为当前生效架构。
...
- `backend/src/main/java/com/knowledgebox/config/KnowledgeBoxProperties.java`
...
- `backend/src/main/java/com/knowledgebox/config/SecurityConfig.java`
