...
- AgentScope 1.0.9 的 `SubAgentTool` 默认必填入参名是 `message`（可选 `session_id`），不是常见的 `query`；若让父 Agent 直接调用原生子 Agent Tool，需做 `query -> message` 兼容或明确约束提示词，否则容易报“找到 null，必须是 string”。
...
- 聊天链路中的知识库能力不是默认内建能力；只有当前 Agent 版本实际绑定了 `KnowledgeBaseSearchTool`，才会在该轮把 `searchKnowledgeBase` 注册进 Agent 工具集；若希望 MAIN 入口先检索再回答，需直接在该版本 `systemPrompt` 中写明。
